...
document.querySelector("img").classList.add("float-right");
...
function renderPaletteRow(colors, parent) {
  ...
  colors.forEach((color) => {
    let box = createColorBox(color, getElementWidth(parent) / colors.length);
...
function renderPaletteTable() {
  ...
  for (let key in COLORS_9) {
    renderPaletteRow(COLORS_9[key], main);
  }
}
renderPaletteTable();
